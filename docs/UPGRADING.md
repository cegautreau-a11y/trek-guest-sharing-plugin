...
## Refreshable guest links in 1.2.2

Version 1.2.2 keeps the complete owner-generated Guest Portal URL fragment after session establishment instead of rewriting the browser back to the bare Guest Portal root. This means:
...
Upload `trek-guest-portal-1.2.2.zip` through **TREK → Admin → Plugins** and enable/update Guest Portal.
...
unzip trek-guest-portal-companion-1.2.2.zip -d /tmp/trek-guest-portal-new
```
