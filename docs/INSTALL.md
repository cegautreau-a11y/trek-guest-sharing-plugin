...
This is the complete first-time installation procedure for TREK Guest Portal 2.0.3 using Docker Compose.
...
- `trek-guest-portal-2.0.3.zip` is uploaded directly to TREK. Do not unzip it first.
- `trek-guest-portal-companion-2.0.3.zip` is extracted on the Docker host.
...
3. Choose **Upload**.
4. Select `trek-guest-portal-2.0.3.zip`.
...
sudo unzip trek-guest-portal-companion-2.0.3.zip -d /opt/trek-guest-portal
...
```text
INFO startup version=2.0.3 ... public_origin=https://guest.example.com trek_public_origin=https://trek.example.com cookie_path=/
```
