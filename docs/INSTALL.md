...
This is the complete first-time installation procedure for TREK Guest Portal 3.5.14 using Docker Compose.
...
```text
trek-guest-portal-3.5.14.zip
trek-guest-portal-companion-3.5.14.zip
```

- `trek-guest-portal-3.5.14.zip` is uploaded directly to TREK. Do not unzip it first.
- `trek-guest-portal-companion-3.5.14.zip` is extracted on the Docker host.
...
3. Choose **Upload**.
4. Select `trek-guest-portal-3.5.14.zip`.
...
sudo unzip trek-guest-portal-companion-3.5.14.zip -d /opt/trek-guest-portal
...
```text
INFO startup version=3.5.14 ... public_origin=https://guest.example.com trek_public_origin=https://trek.example.com cookie_path=/
```
