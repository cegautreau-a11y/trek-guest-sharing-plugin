...
Guest Portal intentionally exposes five sections when the underlying TREK shares are enabled:
...
TREK Guest Portal companion container
       ...
       ├── renders Plan / Flights / Cars & Taxis / Reservations / Photos / Calendar
       ├── serves iCal/WebCal feed with per-event timezone support
...
```text
trek-guest-portal-3.6.1.zip
trek-guest-portal-companion-3.6.1.zip
trek-guest-portal-3.6.1-complete-bundle.zip
```

- `trek-guest-portal-3.6.1.zip` — upload directly through **TREK → Admin → Plugins**.
- `trek-guest-portal-companion-3.6.1.zip` — extract on the Docker host and deploy with `docker compose`.
- `trek-guest-portal-3.6.1-complete-bundle.zip` — plugin, companion and repository documentation in one archive.
